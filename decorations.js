...
function onClick(e) {
    ...
    for (let ball of balls) {
        if (Math.sqrt((x - ball.x) ** 2 + (y - ball.y) ** 2) < ball.radius + radius) {
            return;
        }
    }
    let ball = new Ball(
    ...
    for (let other of balls) {
        ...
        if (
            ...
            if (dist <= ball.radius + other.radius) {
                ...
                let colPointX =
                    (ball.x * other.radius + other.x * ball.radius) / (ball.radius + other.radius);
                let colPointY =
                    (ball.y * other.radius + other.y * ball.radius) / (ball.radius + other.radius);

                //stoping overlap
                ball.x = colPointX + (ball.radius * (ball.x - other.x)) / Math.floor(dist);
                ball.y = colPointY + (ball.radius * (ball.y - other.y)) / Math.floor(dist);
                other.x = colPointX + (other.radius * (other.x - ball.x)) / Math.floor(dist);
                other.y = colPointY + (other.radius * (other.y - ball.y)) / Math.floor(dist);
